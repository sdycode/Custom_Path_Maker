...
class GradProvider with ChangeNotifier {
  ...
  sortColorStopModelsAsPerStopValue(int ind, double sliderTotalWidth,
      {bool ascending = true}) {
    // colorStopModels.sort((a, b) {
    //   return a.colorStop.compareTo(b.colorStop);
    // });
    colorStopModels = pathModels[pathModelIndex].colorStopModels;
...
//  return;
      ...
      if (cmodel.colorStop < colorStopModels[i].colorStop) {
        if (i < ind) {
          colorStopModels.insert(
              i,
              ColorStopModel(cmodel.colorStop, cmodel.hexColorString,
                  sliderTotalWidth * cmodel.colorStop));
    ...
    pathModels[pathModelIndex].colorStopModels = colorStopModels;
    return;
  }
